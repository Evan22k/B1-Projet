...
// 1. On fait la requête UNE SEULE FOIS pour récupérer les infos de la personne connectée
$stmt = $pdo->prepare("
...
// Si l'utilisateur n'existe pas ou n'est pas admin, on le renvoie à la liste
if (!$utilisateur || strtolower($utilisateur['nomRole']) !== 'admin') {
    header("Location: listePC.php?utilisateur=$userId&token=$token");
    exit;
}

// On récupère la liste des rôles disponibles pour le menu déroulant
